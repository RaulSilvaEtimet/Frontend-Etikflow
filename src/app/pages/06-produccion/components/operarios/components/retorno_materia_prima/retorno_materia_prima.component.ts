...
@Component({
    selector: 'app-produccion-operarios-retorno-materia-prima',
    templateUrl: './retorno_materia_prima.component.html',
})
export class ProduccionOperariosRetornoMateriaPrimaComponent {
                    ...
                    if (resp.success) {
                        if (resp.data.length === 1) {
                            if (resp.data[0].IdEstadoOrdenProduccion === 5) {
                                this.onGetAllMateriaPrima(resp.data[0]);
                            } else {
    ...
    onGetAllMateriaPrima(info: ProduccionOrdenProduccionInterface) {
        const parametros = {
            codigo: 1160,
            parametros: {
    ...
    onCalculateMetros(value: number) {
